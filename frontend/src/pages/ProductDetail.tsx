...
import { ArrowLeft, ShoppingCart, Plus, Minus, MessageCircle, Info, CheckCircle2, Share2 } from 'lucide-react';
...
export default function ProductDetail() {
    ...
    fetch(`/api/products/${id}`)
      ...
      .then(data => {
        ...
        fetch('/api/products')
          .then(res => res.json())
          .then(allProducts => {
            setRelatedProducts(allProducts.filter((p: any) => p.category === data.category && p.id !== data.id).slice(0, 4));
            setLoading(false);
          });
      })
      .catch(err => {
  ...
  const handleShare = async () => {
    ...
    const shareTitle = `Découvrez ${displayName} sur Production Lakay !`;
    const shareText = `${displayDescription ? displayDescription.slice(0, 120) + '... ' : ''}Commandez ce produit savoureux sur Production Lakay.`;
    ...
    if (navigator.share) {
      try {
        ...
        const imgUrl = mainImageUrl || product.image_url;
        if (imgUrl && navigator.canShare) {
          try {
            const response = await fetch(imgUrl);
            const blob = await response.blob();
            const file = new File([blob], `${product.id || 'product'}.jpg`, { type: blob.type || 'image/jpeg' });
            if (navigator.canShare({ files: [file] })) {
              shareData.files = [file];
            }
          } catch (e) {
            console.warn("Could not attach image file to share:", e);
          }
        }
      ...
      alert("Lien et détails du produit copiés dans le presse-papier !");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-28 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-2xl shadow-slate-200/50 flex flex-col lg:flex-row border border-slate-100 mb-20">
          ...
          <div className="lg:w-3/5 p-3 sm:p-8 flex flex-col sm:flex-row gap-4 sm:gap-6">
            ...
            <div className="order-1 sm:order-2 flex-grow aspect-square sm:aspect-auto sm:h-[600px] relative rounded-2xl sm:rounded-[2rem] overflow-hidden bg-slate-50">
              <img 
                src={mainImageUrl || product.image_url || '/static/placeholder.jpg'} 
                alt={displayName} 
          ...
          <div className="lg:w-2/5 p-6 sm:p-12 lg:pl-0 flex flex-col">
            ...
            <div className="space-y-8 sm:space-y-10 mt-auto">
              ...
              <div className="flex gap-3 sm:gap-4 pt-2 sm:pt-4">
                ...
                <a 
                  href={`https://wa.me/${settings?.whatsapp_number || '50941103400'}?text=Je souhaite commander: ${displayName}`}
                  onClick={trackClick}
                  target="_blank"
                ...
                <button
                  onClick={handleShare}
                  className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-100 text-slate-700 rounded-2xl sm:rounded-3xl flex items-center justify-center hover:bg-brand-green hover:text-white transition-all shadow-lg active:scale-95 group"
                  title="Partager le produit"
                >
                  <Share2 size={24} className="sm:w-7 sm:h-7 group-hover:scale-110 transition-transform" />
                </button>
              </div>
            </div>
          </div>
