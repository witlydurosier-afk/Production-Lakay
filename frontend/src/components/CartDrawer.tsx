...
export default function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const handleCheckout = () => {
    ...
    items.forEach(i => {
      fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          product_name: i.name,
          category: 'Cart Order', // Simplification or we need category in CartItem
          price: i.price
        })
    ...
    const message = encodeURIComponent(
      `Salut ${settings?.site_name || 'Production Lakay'}! 🌴🥥\n\nNouvelle commande de votre site web:\n\n${list}\n\n━━━━━━━━━━━━━━━━━━━━\n*TOTAL: ${formatCurrency(totalPrice)}*\n━━━━━━━━━━━━━━━━━━━━\n\nMerci de me confirmer la réception !`
    );
    window.open(`https://wa.me/${settings?.whatsapp_number || '50941103400'}?text=${message}`, '_blank');
  };
              ...
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-6">
                  <div className="relative">
                    <div className="w-32 h-32 bg-slate-50 rounded-3xl flex items-center justify-center text-slate-200">
                      <ShoppingBag size={64} />
                    </div>
                    <motion.div 
